...
private fun SettingsCategoryCard(
    ...
    Box(
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            Icon(Icons.Default.ChevronRight, null, tint = CometTail, modifier = Modifier.size(22.dp))
        }
    }
}
        ...
        AnimatedVisibility(visible = settings.runInBackground) {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                var batteryExempt by remember {
                    mutableStateOf(com.gotohex.rdp.util.PowerOptimizationHelper.isIgnoringBatteryOptimizations(context))
                }
                val lifecycleOwner = androidx.compose.ui.platform.LocalLifecycleOwner.current
                DisposableEffect(lifecycleOwner) {
                    val observer = androidx.lifecycle.LifecycleEventObserver { _, event ->
                        if (event == androidx.lifecycle.Lifecycle.Event.ON_RESUME) {
                            batteryExempt = com.gotohex.rdp.util.PowerOptimizationHelper.isIgnoringBatteryOptimizations(context)
                        }
                    }
                ...
                SettingsItem(
                    ...
                    onClick = {
                        if (!batteryExempt) {
                            com.gotohex.rdp.util.PowerOptimizationHelper.launchSafely(
                                context,
                                com.gotohex.rdp.util.PowerOptimizationHelper.ignoreBatteryOptimizationsIntent(context)
                            )
                        }
                    }
                )

                if (com.gotohex.rdp.util.PowerOptimizationHelper.autostartIntent(context) != null) {
                    SettingsItem(
                        ...
                        onClick = {
                            com.gotohex.rdp.util.PowerOptimizationHelper.autostartIntent(context)?.let {
                                com.gotohex.rdp.util.PowerOptimizationHelper.launchSafely(context, it)
                            }
                        }
                    )
                }
...
fun SettingsDataScreen(
    ...
    SettingsSubScaffold(title = stringResource(R.string.data_section), navController = navController) {
        SettingsItem(
...
fun SettingsAboutScreen(
    ...
    SettingsSubScaffold(title = stringResource(R.string.about), navController = navController) {
        SettingsItem(
            ...
            onClick  = {
                com.gotohex.rdp.ui.components.safeOpenUrl(context, "https://t.me/GoToHEX")
            },
        ...
        SettingsItem(
            ...
            onClick  = {
                com.gotohex.rdp.ui.components.safeOpenUrl(context, "https://youtube.com/@dev-hex404")
            },
...
fun PinSetupDialog(
    ...
    var pin1Visible by remember { mutableStateOf(false) }
    var pin2Visible by remember { mutableStateOf(false) }
    ...
    val pin2Focus     = remember { FocusRequester() }

    fun attemptSave() {
        error = when {
            pin1.length < 4                                   -> strTooShort
            pin1 in WEAK_PINS || isSequential(pin1)            -> strWeak
            pin1 != pin2                                       -> strMismatch
            else -> { onConfirm(pin1); "" }
        }
    }
        ...
        text    = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                OutlinedTextField(
                    value         = pin1,
                    onValueChange = {
                        if (it.length <= 6 && it.all(Char::isDigit)) pin1 = it
                        if (error.isNotBlank()) error = "" // clear stale error as soon as the user edits
                    },
                    label         = { Text(stringResource(R.string.pin_label), color = CometTail) },
                    visualTransformation = if (pin1Visible) VisualTransformation.None
                    ...
                    keyboardActions = KeyboardActions(
                        onNext = { pin2Focus.requestFocus() }
                    ),
                    trailingIcon = {
                        IconButton(onClick = { pin1Visible = !pin1Visible }) {
                            Icon(
                                if (pin1Visible) Icons.Outlined.VisibilityOff else Icons.Outlined.Visibility,
                                contentDescription = stringResource(
                                    if (pin1Visible) R.string.cd_hide_pin else R.string.cd_show_pin
                                ),
                                tint = CometTail
                            )
                        }
                ...
                OutlinedTextField(
                    value         = pin2,
                    onValueChange = {
                        if (it.length <= 6 && it.all(Char::isDigit)) pin2 = it
                        if (error.isNotBlank()) error = ""
                    },
                    label         = { Text(stringResource(R.string.pin_confirm_label), color = CometTail) },
                    visualTransformation = if (pin2Visible) VisualTransformation.None
                                           else PasswordVisualTransformation(),
                    keyboardOptions = KeyboardOptions(
                        keyboardType = KeyboardType.NumberPassword,
                    ...
                    trailingIcon = {
                        IconButton(onClick = { pin2Visible = !pin2Visible }) {
                            Icon(
                                if (pin2Visible) Icons.Outlined.VisibilityOff else Icons.Outlined.Visibility,
                                contentDescription = stringResource(
                                    if (pin2Visible) R.string.cd_hide_pin else R.string.cd_show_pin
                                ),
                                tint = CometTail
                            )
                        }
